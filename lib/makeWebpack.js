...
const makeWebpack = (options) => {
  ...
  let spritesConfig = {
    ...
    // spritePath: path.join('assets', 'sprite')
  }
  ...
  let config = {
    ...
    module: {
      rules: [
        ...
        },
        {
          ...
          loader: 'url-loader',
          options: {
            name: mode === 0 ? '[name].[ext]?v=[hash:5]' : path.join(outputPath, 'assets/[name].[ext]'),
        ...
        },
        {
          ...
          loader: "url-loader",
          options: {
            name: mode === 0 ? '[name].[ext]?v=[hash:5]' : path.join(outputPath, 'assets/font/[name].[ext]'),
    ...
    plugins: [
      new MiniCssExtractPlugin({
        filename: mode === 0 ? "[name].css?v=[contenthash:8]" : path.join(outputPath, "css/[name].css")
      }),
  ...
  if (mode === 0) {
    ...
    config.output.filename = path.join(outputPath, "js/[name].js")
  }
